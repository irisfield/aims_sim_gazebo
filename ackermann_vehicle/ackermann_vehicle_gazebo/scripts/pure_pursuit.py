...
class PurePursuit:
    ...
    def __init__(self, path, goal_state, Kp=0.2, ld=0.1, Kf=0.1, max_vel=0.2, max_ang_vel=0.3):
        ...
        self.vehicle_listener = TransformListener()
        self.vehicle_listener.waitForTransform("/map", "/car1/base_link", rospy.Time(0), rospy.Duration(2.0))

        #self.twist_pub_1 = rospy.Publisher('cmd_vel', Twist, queue_size=1)
        self.twist_pub_1 = rospy.Publisher("/car1/twist_cmd", Twist, queue_size=1)
        self.target_point_pub = rospy.Publisher('/car1/pure_pursuit/target_point', PointStamped, queue_size=1)
    ...
    def track_path(self):
        ...
        while(True):
            try:
                (trans, quat) = self.vehicle_listener.lookupTransform("/map", "/car1/base_link", rospy.Time.now())
            ...
            vel_msg_1 = Twist()
            vel_msg_1.linear.x = self.max_vel
            vel_msg_1.angular.z = delta
